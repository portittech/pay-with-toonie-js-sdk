<script>
  ...
  // TODO: use a parameter passed by the app using this sdk to show PWT/Card payment/Stream payment button(s)

  const PUBLIC_STRIPE_KEY = "pk_test_WYMV5CjO1liGYZDEcOHLQQmV00q5lBXeol" // TODO: move to an env file
  ...
  const onButtonClick = async (event) => {
    ...
    const selectedPaymentType = event.target.getAttribute("id")
    ...
    try {
      if (selectedPaymentType === "stream") {
        try {
          ...
          if (streamPaymentData.paymentIntentStreamId) {
            loadDataError = false;
          } else {
            loadDataError = true;
            console.warn('Error on Load Data: paymentIntentStreamId missing')
          }

          qrCodeValue = JSON.stringify({
            paymentSessionId: streamPaymentData.paymentIntentStreamId,
...
<div>
    <button on:click={onButtonClick} class="primary-btn" id="pwt">
    ...
    <button on:click={onButtonClick} class="primary-btn" id="stream">
    ...
    {#if stripe}
        <Elements {stripe}>
            <button on:click={onButtonClick} class="primary-btn primary-btn--card" id="card">
